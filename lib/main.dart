...
void _liveRenderIsolate(SendPort mainPort) {
  ...
  port.listen((message) {
    if (message is EngineConfig) {
      ...
      int reportStep = math.max(1, config.renderR ~/ 15); 
      ...
      for (int r = 0; r < config.renderR; r++) {
        ...
        // Stream partial pixels back to UI thread
...
Future<Uint8List> _backgroundExportTask(ExportConfig eCfg) async {
  if (eCfg.isGif) {
    ...
    for (int f = 0; f < eCfg.frames; f++) {
      ...
      final bigB = BigInt.from(modEngine.bNum);
      
      for (int r = 0; r < modEngine.renderR; r++) {
        for (int c = 0; c < modEngine.renderC; c++) {
          BigInt n = modEngine.startN + BigInt.from(r * modEngine.renderC + c);
          int result = 0;

          if (modEngine.postType == 'NONE') {
            result = _solveForN(n, bigB, modEngine);
          } else if (modEngine.postType == 'ITERATE') {
            result = _solveForN(n, bigB, modEngine);
            for (int i = 1; i < modEngine.iterK; i++) {
              result = _solveForN(BigInt.from(result), bigB, modEngine);
            }
          }

          int colorData = modEngine.palette[result % modEngine.modM];
    ...
    final bigB = BigInt.from(eCfg.engine.bNum);

    for (int r = 0; r < eCfg.engine.renderR; r++) {
      for (int c = 0; c < eCfg.engine.renderC; c++) {
        BigInt n = eCfg.engine.startN + BigInt.from(r * eCfg.engine.renderC + c);
        int result = 0;

        if (eCfg.engine.postType == 'NONE') {
          result = _solveForN(n, bigB, eCfg.engine);
        } else if (eCfg.engine.postType == 'ITERATE') {
          result = _solveForN(n, bigB, eCfg.engine);
          for (int i = 1; i < eCfg.engine.iterK; i++) {
            result = _solveForN(BigInt.from(result), bigB, eCfg.engine);
          }
        }

        int colorData = eCfg.engine.palette[result % eCfg.engine.modM];
        ...
        for (int i = 1; i <= loopEnd; i++) {
          ...
          for (int Ci = x1; Ci <= Sigma; Ci++) {
            ...
            for (int xNext = 0; xNext < b; xNext++) {
              ...
              if (config.logicOp == 2) conditionMet = c1 || _evaluateRHS(config.rhs2Rule, val, di, dNext, b);
...
class _SequencerWorkspaceState extends State<SequencerWorkspace> {
  ...
  final List<String> _lhsLabels = [
    "| xi - x(i+1) |", "Anchor | x1 - x(i+1) |", "(x(i+1) - xi + b) % b", "(xi + x(i+1)) % b", "max(xi, x(i+1))", "min(xi, x(i+1))",
    "xi ⊕ x(i+1)", "Accumulator Drop", "(xi × x(i+1)) % b", "|(b - 1 - xi) - x(i+1)|", "Lookback |xi-1 - xi|", "Lookahead |xi - x(i+2)|",
    "|(xi)² % b - x(i+1)|", "[OMITTED]", "Global Structure", "Complex Z Exact", "Complex Z Floor", "Complex Z Ceil", "Complex Z Round",
    "Sum of Cubes", "Diff of Squares", "GCD", "LCM", "Center Dist", "Bitwise OR", "Bitwise AND", "L-Shift XOR",
    "Triangular Num", "Quadratic Form", "D-weighted sum", "Shifted Product"
  ];

  final List<String> _rhsLabels = [
    "= di", "= b - 1 - di", "≤ di", "≥ di", "≠ di", "≡ di (mod 2)", "= floor(b/2)", "< di", "> di", "= (di + 1) % b",
    "= (di + d(i+1)) % b", "= (di × d(i+1)) % b", "≡ di (mod 3)", "≡ di (mod 4)", "= (di)² % b", "= |di - d(i+1)|",
    "= max(di, d(i+1))", "= min(di, d(i+1))", "= di ⊕ d(i+1)", "> floor(b/2)", "< floor(b/2)", "= (b - di) % b",
    "≠ (di + 1) % b", "≤ (di + b/2) % b", "≥ (di - b/2) % b"
  ];
  ...
  void _debouncedRender() {
    ...
    _debounceTimer = Timer(const Duration(milliseconds: 250), _triggerRender);
  }
  ...
  Future<void> _exportArtifact(bool asGif) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
      content: Text('Processing native export... This may take a moment.'),
    ...
    try {
      ...
      Uint8List resultBytes = await Isolate.run(() => _backgroundExportTask(eCfg));
      ...
      final xfile = XFile(file.path, mimeType: asGif ? 'image/gif' : 'image/png');
      await Share.shareXFiles([xfile], text: 'Universal Sequence Render');
      
    } catch (e) {
      if (mounted) {
        setState(() { _isProcessing = false; });
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('Export Failed: Memory limit exceeded. Try a smaller custom resolution.'),
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        Expanded(child: _buildNumInput('BASE', _bNum, (v) { _bNum = v; _debouncedRender(); })),
                        const SizedBox(width: 8),
                        Expanded(child: _buildNumInput('MOD', _modM, (v) { _modM = v; _generatePalette(); _debouncedRender(); })),
                        const SizedBox(width: 8),
                        Expanded(child: _buildNumInput('RANGE', _pVal, (v) { _pVal = v; _debouncedRender(); })),
                        const SizedBox(width: 8),
                        Expanded(child: _buildDropdown('GRID', _viewScaleIdx - 1, 3, (v) { setState(() => _viewScaleIdx = v! + 1); _debouncedRender(); }, customLabels: ['b', 'b²', 'b³', 'Custom'])),
                      ],
                    ),
                    if (_viewScaleIdx == 4) ...[
                      const SizedBox(height: 8),
                      _buildNumInput('CUSTOM RESOLUTION (MAX 16384)', _customViewDim, (v) { _customViewDim = v; _debouncedRender(); }),
                    ],
                    ...
                    Row(
                      ...
                      children: [
                        Expanded(flex: 2, child: _buildDropdown('LHS', _lhsRule, _lhsLabels.length - 1, (v) { setState(() => _lhsRule = v!); _debouncedRender(); }, customLabels: _lhsLabels)),
                        const SizedBox(width: 8),
                        Expanded(flex: 2, child: _buildDropdown('RHS 1', _rhs1Rule, _rhsLabels.length - 1, (v) { setState(() => _rhs1Rule = v!); _debouncedRender(); }, customLabels: _rhsLabels)),
                        const SizedBox(width: 8),
                        SizedBox(
                          height: 48, width: 48,
                          child: ElevatedButton(
                            style: ElevatedButton.styleFrom(
                              ...
                              backgroundColor: _logicOp == 2 ? Theme.of(context).colorScheme.primary : const Color(0xFF2A2A2A),
                            ),
                            onPressed: () => setState(() { _logicOp = _logicOp == 2 ? 0 : 2; _debouncedRender(); }),
                            child: Icon(_logicOp == 2 ? Icons.add_circle : Icons.add, color: Colors.white),
                          ),
                        ),
                        const SizedBox(width: 8),
                        Expanded(
                          ...
                          child: Opacity(
                            ...
                            child: IgnorePointer(
                              ignoring: _logicOp != 2,
                              child: _buildDropdown('RHS 2', _rhs2Rule, _rhsLabels.length - 1, (v) { setState(() => _rhs2Rule = v!); _debouncedRender(); }, customLabels: _rhsLabels)
                            ),
                          )
                        ),
                      ],
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _buildDropdown('TRANSFORMATION', ['NONE', 'ITERATE', 'C_SEQ', 'D_SEQ'].indexOf(_postType), 3, (v) { 
                            setState(() => _postType = ['NONE', 'ITERATE', 'C_SEQ', 'D_SEQ'][v!]); _debouncedRender(); 
                          }, customLabels: ['Raw', 'Iterate a(n)', 'C(n) Congruence', 'D(n) Diagonal Sum']),
                          
                          if (_postType == 'ITERATE') ...[
                            const SizedBox(height: 8),
                            _buildNumInput('ITERATION DEPTH (k)', _iterK, (v) { _iterK = v; _debouncedRender(); }),
                          ],
                          ...
                          if (_postType == 'C_SEQ' || _postType == 'D_SEQ') ...[
                            ...
                            Row(
                              children: [
                                Expanded(child: _buildDropdown('LOCAL MAP', _postGridScaleIdx - 1, 3, (v) { setState(() => _postGridScaleIdx = v! + 1); _debouncedRender(); }, customLabels: ['b', 'b²', 'b³', 'Custom'])),
                                if (_postGridScaleIdx == 4) ...[
                                  const SizedBox(width: 8),
                                  Expanded(child: _buildNumInput('ROWS', _customPostR, (v) { _customPostR = v; _debouncedRender(); })),
                                  const SizedBox(width: 8),
                                  Expanded(child: _buildNumInput('COLS', _customPostC, (v) { _customPostC = v; _debouncedRender(); })),
                                ]
                              ],
                            ),
                          ],
                          ...
                          if (_postType == 'C_SEQ') ...[
                            ...
                            Row(
                              children: [
                                Expanded(child: _buildNumInput('TARGET (T)', _targetT, (v) { _targetT = v; _debouncedRender(); })),
                                const SizedBox(width: 8),
                                Expanded(child: _buildNumInput('MOD (Mc)', _modMc, (v) { _modMc = v; _debouncedRender(); })),
                              ],
                            )
                          ],
                        ],
                    ...
                    Row(
                      children: [
                        Expanded(child: _buildNumInput('TUPLE DEPTH', _tupleK, (v) { _tupleK = v; _debouncedRender(); })),
                        const SizedBox(width: 8),
